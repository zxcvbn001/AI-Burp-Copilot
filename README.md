...
修改规则后建议先在“验证工作台”单点验证，再开启自动验证。内置规则位于 `src/main/resources/rules/payloads/`，外置规则会覆盖同 `attackType` 的内置规则。
...
“验证工作台”用于人工选择参数和漏洞类型，然后执行最小化验证。

适合以下场景：

- AI 建议不稳定，需要人工控制。
- 只想验证某个参数。
- 只想跑 SQLI、XSS、IDOR 等某一类规则。
- 想查看完整 Request/Response 证据链。
